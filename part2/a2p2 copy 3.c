...
// SERVER ----------------------------------------------------
    ...
    while (1)
    {
        ...
        // DONE?: Receive
        // Source: eClass: poll.c
        int num_events = poll(server_rcv_pfds, NCLIENT, timeout);
        if (num_events == -1) {
            perror("Error: Poll failed\n");
            exit(EXIT_FAILURE);
        }
        // Source: eClass: poll.c
        for (int client_id = 0; client_id < NCLIENT; client_id++) {
            // TODO: Add debug lines
            
            // Check for any received events
			if (server_rcv_pfds[client_id].revents & POLLIN) 
			{
                // Event received from client[client_id]
                ...
                switch (client_id) {
                    case 1:
                        received_packet = rcvPacket(fifo_c1_s_fd);
                    case 2:
                        received_packet = rcvPacket(fifo_c2_s_fd);
                    case 3:
                        received_packet = rcvPacket(fifo_c3_s_fd);
                }
                printf("  Packet type: %d", received_packet.packet_type);
				...
				// int buffer_len = read(server_rcv_fds[client_id], packet_buffer, BUFSIZE);
